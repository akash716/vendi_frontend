...
export default function Candies() {
  ...
  const deleteCandy = async (candy) => {
    if (
      !window.confirm(
        `Delete "${candy.name}"?\n\nIf it's in a candy list, it cannot be deleted — remove it from lists first.`
      )
    )
      return;
  ...
  const startEdit = (c) => {
    setEditId(c.id);
    setEditName(c.name);
    setEditCategory(c.category || categories[0]?.name || "");
  ...
  const addCategory = async () => {
    if (!newCatName.trim() || !newCatPrefix.trim())
      return alert("Name and prefix required");
    setCatLoading(true);
    const res  = await adminFetch(CAT_API, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name:   newCatName.trim(),
        prefix: newCatPrefix.trim().toUpperCase(),
      }),
    });
    const data = await res.json();
    setCatLoading(false);
  ...
  const deleteCategory = async (cat) => {
    if (
      !window.confirm(
        `Delete category "${cat.name}"?\nOnly works if no candies are in it.`
      )
    )
      return;
  ...
  const displayed =
    filterCat === "ALL"
      ? candies
      : candies.filter((c) => c.category === filterCat);
  ...
  return (
    <div style={{ maxWidth: 960 }}>
      ...
      >
        ...
        <button onClick={() => setShowCatModal(true)} style={ghostBtn}>
          ⚙️ Manage Categories
        </button>
      </div>
      ...
      {/* ── Add Candy Form ── */}
      <div style={card}>
        <div style={{ fontWeight: 700, marginBottom: 10, fontSize: 14, color: "var(--cream4)" }}>
          ➕ Add New Candy
        </div>
        <div
          style={{
        ...
        <button onClick={createCandy} style={goldBtn}>
          + Add Candy
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
        <button
          ...
          style={{
            ...pill,
            background: filterCat === "ALL" ? "var(--gold2)" : "transparent",
            color: filterCat === "ALL" ? "var(--bg0)" : "var(--cream1)",
            borderColor: filterCat === "ALL" ? "var(--gold2)" : "var(--border1)",
          }}
        >
          All ({candies.length})
        </button>
                  ...
                  <>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 2 }}>
                      <button
                        ...
                        style={{ ...goldBtn, flex: 1, padding: "7px", fontSize: 12 }}
                      >
                        Save
                      </button>
                      <button
                        onClick={() => setEditId(null)}
                        style={{ ...ghostBtn, flex: 1, padding: "7px", fontSize: 12 }}
                      >
                        Cancel
                      </button>
                    </div>
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: 18 }}>
              {categories.length === 0 && (
                <p style={{ color: "var(--cream0)", fontSize: 13 }}>No categories yet.</p>
              )}
              {categories.map((cat, idx) => {
                const b     = bc(idx);
                const count = candies.filter((c) => c.category === cat.name).length;
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ fontSize: 11, color: "var(--cream0)", whiteSpace: "nowrap" }}>
                        {count} candies
                      </span>
                    </div>
                    <button
